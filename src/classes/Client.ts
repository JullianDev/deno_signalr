...
> {
  ...
  */
   ...
   * @param protocol - he SignalR client protocol
  ...
  public start(protocol = 1.5): void {
    if (!this._bound) {
      ...
      if (!(this.url.startsWith("http:") || this.url.startsWith("https:"))) {
        return this._error(ErrorCode.invalidProtocol);
      }
    ...
    this._negotiate(protocol).then(
      (negotiateProtocol: Record<string, unknown>) => {
        if (
          this.connection &&
